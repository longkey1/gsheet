...
func runCellsUpdate(cmd *cobra.Command, args []string) error {
	...
	values, err := parseInputValues(filePath, useStdin, valuesStr, true)
	if err != nil {
		return err
	}
...
func runCellsNoteSet(cmd *cobra.Command, args []string) error {
	...
	if err := gsheet.UpdateNote(svc.Sheets.Service, spreadsheetID, sheetID, cell, noteText); err != nil {
		return err
	}
...
func runCellsImport(cmd *cobra.Command, args []string) error {
	...
	values, err := parseInputValues(filePath, useStdin, "", false)
	if err != nil {
		return err
	}
